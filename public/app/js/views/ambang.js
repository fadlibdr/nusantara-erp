...
/** Sel rupiah yang boleh kosong — digaris, tidak dinolkan. */
function selRupiah(value) {
  if (value === null || value === undefined) return el('td.right', el('span.cell-sub', { text: '—' }));
  return el('td.right.num', { text: fmt.rupiah(value) });
}
    ...
    el('div', [
      ...
      el('.desc', {
        text: 'Angka yang mendekati atau melewati batasnya — anggaran proyek terhadap RAP, RAP terhadap '
          + 'nilai kontrak, realisasi overhead terhadap OVB — dihitung langsung dari data hari ini.',
      }),
    ]),
  ...
  measures.forEach((measure) => {
    ...
    body.appendChild(el('.card', [
      el('.card-head', [
        ...
        el('span', [
          badge(`Peringatan ≥ ${fmt.percent(measure.warn_pct, { decimals: 0 })}`, 'amber'),
          el('span.cell-sub', {
