...
| jam tenang menunda | ✅ `outbox` delay | ✅ job pengganti dengan sisa jadwal (bukan `release()`) | — | ✅ `retry` delay + kalimat | `next_attempt_at` | ✅ kolom Berikutnya + alert Profil | — | — |
...
- Job yang dilepas `release()` oleh jam tenang menaikkan hitungan percobaan PEKERJA (bukan
  `attempts` baris). Semula saya tulis "teoretis — lima malam berturut-turut"; verifikasi
  12 Sep 2026 (B-1) mengukur bahwa **satu malam gangguan SMTP sementara sudah cukup**: empat
  kegagalan dalam 21 menit (21:30 → 21:51, backoff 60/300/900 s) lalu backoff 3600 s mendarat
  22:51 — di dalam 22:00–06:00 — `release()` sampai 06:00, dan pukul 06:00 pekerja menggagalkan
  job SEBELUM `handle()` berjalan (`attempts` job 6 > 5): baris `failed` "Percobaan habis …
  Pesan penyedia terakhir: Ditunda oleh jam tenang …" dengan hanya **4** panggilan kanal.
  Jendela pemicunya: kegagalan sementara pertama antara ±20:39 dan ±04:39 WIB pada penerima
  berjam-tenang 22:00–06:00. **Ditutup**: job yang tertunda selesai dan mewariskan sisa
  jadwalnya kepada job pengganti (`$priorAttempts`, `$tries` sisa, `backoff()` dipotong) yang
