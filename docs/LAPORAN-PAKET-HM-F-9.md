...
| **DV-F9-7** | `throttle:10,1` pada rute publik **tidak per-klien-sungguhan**: `bootstrap/app.php:17` (pra-ada, di luar diff F-9) berbunyi `$middleware->trustProxies(at: '*')`, jadi embernya dikunci pada IP yang dipercaya dari header dan siapa pun yang boleh menyetel `X-Forwarded-For` mereset kuotanya sendiri. Terukur 10 Sep 2026: sesudah ember polos habis (429, `X-RateLimit-Remaining: 0`), **30 permintaan dengan 30 nilai `X-Forwarded-For` berbeda** semuanya dijawab **404** dengan `remaining: 9`. | **Di luar lingkup F-9, tidak diubah — kalimatnya yang diperbaiki.** Yang membuat penebakan token mustahil adalah entropi `Str::random(40)`, bukan throttle-nya; docblock `Modules/ServiceDesk/Routes/web.php` semula memuji penjaga yang salah dan sekarang menyebut yang sebenarnya. Kondisi ini pra-ada dan berlaku sama pada `/persetujuan/{token}`; menutupnya berarti menyentuh `bootstrap/*`, yang paket ini dilarang menyentuh. Saudara DV-F9-4: keduanya menunggu paket yang menangani proxy tepercaya. |
