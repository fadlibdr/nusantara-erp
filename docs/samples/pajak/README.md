...
| `ebupot_unifikasi_csv` | `ebupot-unifikasi-csv-<YYYY-MM-DD>.<ekstensi asli>` — template impor bukti potong e-Bupot Unifikasi (PPh 23, PPh final 4(2)) | Coretax DJP → e-Bupot Unifikasi → impor bukti potong → unduh template | **Ada** (`TaxExportService::eBupot`) — **belum diverifikasi**; kolomnya harus dicocokkan terhadap berkas ini; baris ber-NPWP 16/22 digit disebut di kartu "Perlu dicocokkan" sampai saat itu |
