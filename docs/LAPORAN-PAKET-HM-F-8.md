...
| D9 | **Sensus "28 jenis dokumen" basi di dokumen HIDUP**: registri `ApprovableDocuments` berisi 29 sejak `d10fcf6` (7 Sep 2026, OVB), dan angka 28 masih terbaca di PANDUAN-PENGGUNA §1.5, ONBOARDING/direktur, ONBOARDING/admin — dan disalin ke `SIKAP-E-SIGN.md` §5.3 yang BARU | `count(ApprovableDocuments::all())` = 29; `git grep -c "=> \[" main -- …/ApprovableDocuments.php` = 29 | **DITUTUP** di empat dokumen hidup itu. **DIBIARKAN** di `LAPORAN-PAKET-HM-F-1`, `ASESMEN-UX-2026-09`, `ANALISIS-PROSES-BISNIS-2026-09` dan baris rencana ROADMAP §3: keempatnya bertanggal (potret/rencana), mengeditnya memalsukan potretnya — alasan yang sama dengan D7 |
| D10 | **ONBOARDING/admin masih menghitung "kesembilan belas pengawas tenggat"** — permukaan KEEMPAT yang luput dari sapuan 18/19 → 34 milik D4 | `count(WatchedDeadlines::entries())` = 34; `erp:deadline-watch` mencetak "Checked 34 watcher(s)" | **DITUTUP** — "ketiga puluh empat" |
