...
- Notes:
  - **What the tree already does, so the question is asked against reality** (read before writing OQ-4): PO Rp 100 jt and SPK Rp 200 jt director thresholds exist and are operator-editable (`approvals.purchase_order.threshold_two_level`, `approvals.subcontract.threshold_two_level` in `SettingService`'s registry, `PurchaseOrder::directorApprovalThreshold()`, enforced by `Procurement\Support\DirectorApproval` — the ONE approval must be a `prc/scm.approve-director` holder); award decisions resolve a three-bracket ladder in `config/erp.php` through `ApprovalLevels` (config-only by its own docblock's reasoning); the other 25 `ApprovableDocuments` types have no threshold at all. Only `prc`/`scm` have `.approve-director` (`PermissionSeeder::DIRECTOR_APPROVALS`, the "+ 2" in 86); `fin.approve-director` does not exist and `fin.approve` is the ordinary finance approval (finance-manager holds it too) — so the entry's acceptance sentence itself is one of the four decisions OQ-4 asks (the numbers per type and value column; one-director-approval vs a second distinct approval; whether a lower `threshold_single` band exists at all and whether it equals T4.3's `batch_cap`; Pengaturan vs config and who holds the new director rights).
  - The RECAP entry's keys (`approvals.threshold_single` / `threshold_director` per prefix in `SettingService`) were NOT pre-created empty: a registry row with no number would print a blank rule in Pengaturan, and `ApprovalLevels::ladder()` documents why the ladder stays out of the settings form — pre-deciding that is the decision itself.
  - ANALISIS E2's "≤ Rp 25 jt satu tingkat, > Rp 250 jt direktur" is quoted in OQ-4 as the analyst's illustration, not as a candidate answer; ASESMEN P2-3's "≤ Rp 25 jt" batch cap is cross-referenced because it is the only other number in the uploads and may be the same one.
...
- Notes:
  - **What exists and what does not** (read before writing OQ-5): no delegation table, no "on behalf of" on `core_approvals` (`action · user_id · note`), `NotificationService::approvers()` = every active permission holder minus the actor, `ApprovalQueue::pending($user)` = permission + not the owner, `SegregationOfDuties` compares approver with submitter only; `hr_leave_requests` (`employee_id · start_date · end_date · status`) and `users.employee_id` are the data an automatic window would read. The production cost of an absent approver is the two `erp:approval-watch` escalations of 4 Sep 2026 (PAY/2026/VIII/0002 33 hari, SPK/2026/III/0002 40 hari); whether absence caused them is not recorded and OQ-5 says so.
