# Cadangan MySQL & drill restore — gladi, 5 September 2026

Bukti T0.6 (ROADMAP-HASHMICRO Fase 0): `deploy/backup-erp1.sh --engine mysql`
dijalankan utuh terhadap `erp_dryrun` (isi = salinan produksi hasil
`erp:sqlite-to-mysql`, lihat migration-verify-dryrun-2026-09-05.md):
mysqldump → gzip → GPG AES-256 → rsync ke tujuan offsite → drill: ambil
artefak terbaru → dekripsi → muat ke `erp_restore_check` → hitung pengguna &
jurnal → `erp:migration-verify` terhadap basis data hidup. Semuanya di
lingkungan coretan: konf `ERP_BACKUP_CONF`, direktori `ERP_BACKUP_DIR`,
status `ERP_BACKUP_STATUS_DIR`, kunci GPG coretan, tujuan offsite
`rsync:root@localhost:<S>/offsite`. Tidak ada yang menyentuh
`/etc/erp1/backup.conf` (offsite produksi masih belum dikonfigurasi —
keputusan pemilik), `/var/backups/erp1`, kunci produksi, cron, maupun
`erp`.

Kredensial: berkas my.cnf `[client]` mode 600 lewat `--defaults-extra-file`
(`deploy/erp1-mysql-backup.cnf.example`), tidak pernah di argv atau
lingkungan.

## 1. Cadangan penuh (lokal + offsite), engine dari konf `BACKUP_ENGINE=mysql`

```
2026-09-05 07:44:07  dumping MySQL schema erp_dryrun
2026-09-05 07:44:08  verified: 190 tables, users rows present
2026-09-05 07:44:08  database -> erp1-db-20260905-074407.sql.gz (72K)
2026-09-05 07:44:09  attachments -> erp1-files-20260905-074407.tar.gz (20K, 72 files)
2026-09-05 07:44:09  local done — 2 files, 96K total
2026-09-05 07:44:10  pushing 2 file(s) to rsync:root@localhost:<S>/offsite
2026-09-05 07:44:11  offsite ok — 2 file(s) at rsync:root@localhost:<S>/offsite, newest 20260905-074407, 2 pushed this run
```

Artefak: `erp1-db-20260905-074407.sql.gz` (72 KB; dump 508 KB, 190 tabel,
trailer `-- Dump completed` diperiksa, baris `users` ada) dan
`erp1-files-…tar.gz`; di offsite keduanya `.gpg`, sha256 dibaca balik.

## 2. Drill restore dari offsite → `erp_restore_check` → verify vs `erp_dryrun`

```
2026-09-05 07:44:12  drill: fetching erp1-db-20260905-074407.sql.gz.gpg (0d old, key 5ed4af2f433ab219)
2026-09-05 07:44:12  verified: 190 tables, users rows present
2026-09-05 07:44:18  drill: loading dump into erp_restore_check
2026-09-05 07:44:31  restored copy verified: 11 users, 16 journals in erp_restore_check
2026-09-05 07:44:33  verify: 183 tabel dibandingkan (7 dilewati), 1421 baris sumber / 1421 baris tujuan, 264 kolom desimal, 0 selisih, 0 tidak diketahui — identik.
2026-09-05 07:44:34  verify report: /root/construction-erp/storage/app/migration-report-20260905-144433.md
2026-09-05 07:44:34  RESTORE DRILL PASSED — erp1-db-20260905-074407.sql.gz.gpg, 0 day(s) old, from rsync:root@localhost:<S>/offsite
```

Status (`offsite-status.json`) sesudahnya: `last_drill_result: passed`,
`engine: mysql`, `key_fingerprint` kunci coretan.

## 3. Drift: satu `users.name` diubah di basis data hidup, drill diulang dari artefak yang sama

```
2026-09-05 07:45:50  verify: 183 tabel dibandingkan (7 dilewati), 1421 baris sumber / 1421 baris tujuan, 264 kolom desimal, 1 selisih, 0 tidak diketahui — BERBEDA.
2026-09-05 07:45:50  verify: 1 table(s) differ from the live database — drift since the dump (0d old), or a problem if nothing was written meanwhile; read the report
2026-09-05 07:45:50  RESTORE DRILL PASSED WITH DRIFT (1 table(s)) — erp1-db-20260905-074407.sql.gz.gpg, 0 day(s) old, from rsync:root@localhost:<S>/offsite
```

`last_drill_result: drift` — restore berhasil, basis data hidup berbeda 1
tabel dari dump. Inilah yang akan terlihat bila drill dijalankan siang hari;
pada jadwal cron (03:30 WIB, 75 menit sesudah cadangan) angkanya harus 0.
`erp:backup-watch` hanya membunyikan alarm pada `failed`. Nama dikembalikan
sesudah uji.

## 4. Drill dari artefak LOKAL (`--source=local`, tanpa offsite, tanpa GPG)

```
2026-09-05 07:45:50  drill: using local erp1-db-20260905-074407.sql.gz (0d old, no GPG on the local copy)
2026-09-05 07:45:53  drill: loading dump into erp_restore_check
2026-09-05 07:46:01  verify: 183 tabel dibandingkan (7 dilewati), 1421 baris sumber / 1421 baris tujuan, 264 kolom desimal, 0 selisih, 0 tidak diketahui — identik.
2026-09-05 07:46:01  RESTORE DRILL PASSED — erp1-db-20260905-074407.sql.gz, 0 day(s) old, from local <S>/backup
```

Berkas status tidak disentuh (status itu tentang salinan offsite). Inilah
mode yang dipakai runbook cut-over selama offsite produksi belum
dikonfigurasi.

## 5. Regresi engine sqlite (`--engine sqlite --local-only`, lalu drill lokal)

```
2026-09-05 07:46:04  snapshotting database
verified: 12 users
2026-09-05 07:46:04  database -> erp1-db-20260905-074604.sqlite.gz (120K)
2026-09-05 07:46:04  attachments -> erp1-files-20260905-074604.tar.gz (20K, 72 files)
2026-09-05 07:46:04  local done — 2 files, 144K total
2026-09-05 07:46:04  drill: using local erp1-db-20260905-074604.sqlite.gz (0d old, no GPG on the local copy)
restored copy verified: 12 users, 7 journals
2026-09-05 07:46:04  RESTORE DRILL PASSED — erp1-db-20260905-074604.sqlite.gz, 0 day(s) old, from local <S>/backup-sqlite
```

Jalur SQLite lama tidak berubah (VACUUM INTO + integrity_check + hitung
pengguna); berkas `database.sqlite` yang dibaca tidak berubah mtime-nya.

## 6. Laporan erp:migration-verify dari drill §2 (erp_dryrun → erp_restore_check)

# Laporan verifikasi migrasi — 2026-09-05 14:44:33

- Sumber: `mysql` (mysql `erp_dryrun`)
- Tujuan: `mysql_restore_check` (mysql `erp_restore_check`)
- **183 tabel dibandingkan**, 1421 baris sumber / 1421 baris tujuan, 264 kolom desimal dijumlahkan, **0 selisih**, 0 tidak diketahui → **identik**
- Dilewati (`--ignore`): `sessions`, `cache`, `cache_locks`, `jobs`, `job_batches`, `failed_jobs`, `password_reset_tokens`
...
| `password_reset_tokens` | | | | | | | dilewati |
