...
> Terverifikasi berjalan: 77 migrasi + seed demo penuh, PHPUnit hijau, dan smoke test
> API (login, seluruh modul, alur create→submit→approve) lulus pada PHP 8.3 / SQLite.
...
```bash
curl -s http://localhost:8000/api/projects/projects \
...
1. ~~Tes otomatis untuk matematika bisnis (PPh 21 TER, moving average, opname subkon,
   laporan keuangan)~~ — **selesai**, `phpunit` menjalankan 1.076 tes.
